...
public class LocationDetailController {
	...
	public String viewDetailPage(HttpServletRequest request, 
		...
		if(isLogin){
			...
			ActUserDTO aDTO = new ActUserDTO(member.getEmail(), ActUserManager.DETAIL_PAGE, locationSeq);
			dto= manager.checkLikeStatus(aDTO, dto);
		}
		
		//디테일  페이지 좋아요 카운트  
		int likeCount = manager.getLikeStatusCount(new ActUserDTO(ActUserManager.DETAIL_PAGE, locationSeq));
